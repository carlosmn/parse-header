...
module CParser
  ...
  class Parser < Parslet::Parser
    ...
    rule(:comment) {
      (str('/**') >> (str('*/').absent? >> any).repeat >> str('*/')).as(:exp_comment) >> spaces? |
    ...
    rule (:shift_constant) {
      (
        left_paren.maybe >> identifier  >> (left_shift | right_shift) >> identifier >> right_paren.maybe
      ).as(:constant) >> spaces?
    }
    ...
    rule(:type_specifier) {
      ...
      enum_specifier |
      # This allows us to have our own types or types from headers
      var_type
    }
...
#          (left_brace >> struct_declaration_list >> right_brace)
    ...
    rule(:enum_specifier) {
      enum_keyword >> (
        (
          (identifier.maybe | spaces?) >> (left_brace >> enumerator_list >> right_brace)
        ) | identifier
      )
    }
